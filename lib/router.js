...
if (Meteor.isClient) {
    ...
    Router.onBeforeAction(filters.canView, {
        except: ['atSignIn', 'atSignUp', 'atForgotPwd', 'atResetPwd', 'signOut']
    });
    ...
    Router.onBeforeAction(filters.canPost, {
        only: ['posts_pending', 'post_submit']
    });
    Router.onBeforeAction(filters.canEditPost, {
        only: ['post_edit']
    });
    Router.onBeforeAction(filters.canEditComment, {
        only: ['comment_edit']
    });
    Router.onBeforeAction(filters.isAdmin, {
        only: ['posts_pending', 'all-users', 'settings', 'toolbox', 'logs']
    });

    // After Hooks

    Router.onAfterAction(filters.resetScroll, {
        except: ['posts_top', 'posts_new', 'posts_best', 'posts_pending', 'posts_category', 'all-users']
    });
...
PostsListController = FastRender.RouteController.extend({
    ...
    data: function() {
        ...
        parameters.find.createdAt = {
            $lte: Session.get('listPopulatedAt')
        };
        var posts = Posts.find(parameters.find, parameters.options);

        // Incoming posts
        parameters.find.createdAt = {
            $gt: Session.get('listPopulatedAt')
        };
...
UserPageController = FastRender.RouteController.extend({
    ...
    data: function() {
        ...
        var findBySlug = Meteor.users.findOne({
            slug: this.params._idOrSlug
        });
    ...
    Router.map(function() {
        ...
        this.route('posts_default', {
            ...
            controller: getDefaultViewController()
                ...
                //       SEO.set({
                //       og: {
                //         'title': 'Are You Making A Difference',
                //         'description': 'Did you ever want to make a difference in the lives of the people around you?'
                //       }
                //     });
                // }
        });
